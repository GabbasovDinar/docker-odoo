...
Set `ODOO_VERSION` to the checked-out branch version. For example, on branch `19.0`:
...
The default configuration builds Odoo 19 from the official GitHub repository:
...
Example for `18.0 -> 19.0`:

```env
ODOO_VERSION=19.0
OPENUPGRADE=True
OPENUPGRADE_SOURCE_DATABASE_NAME=ODOO_18
OPENUPGRADE_TARGET_DATABASE_NAME=ODOO_19
...
```text
ODOO_18    unchanged source database
ODOO_19    copy of ODOO_18, migrated to Odoo 19
```
...
OPENUPGRADE=True
OPENUPGRADE_SOURCE_DATABASE_NAME=production18
OPENUPGRADE_TARGET_DATABASE_NAME=migration19
```
...
+----------------------+            +----------------------+
| pg_dump / pg_restore |----------->| production18         |
| Odoo target version  |            | migration19          |
+----------------------+            +----------------------+
```
...
```bash
rsync -a \
  odoo@old-odoo-vm:/var/lib/odoo/filestore/production18/ \
  /path/to/source-filestore/production18/
```

Then ensure that the content is available inside the migration container as:

```text
/var/lib/odoo/filestore/production18
```
...
- [Odoo 19 testing documentation](https://www.odoo.com/documentation/19.0/developer/reference/backend/testing.html)
